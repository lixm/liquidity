...
Section Liquidity. 
    ...
    | H : rejectItem ?chain ?ctx ?st = Ok (?new_st, ?acts) |- _ =>
        ...
        match type of H with
          context [if ?cond then _ else _] =>
            destruct cond eqn: Ereject in H;
            try discriminate;
            simpl in H
        end
    end.
  ...
  Lemma buyer_seller_neq:
    seller_adr <> buyer_adr.
  Proof.
    unfolds in H_init.
    destruct H_init as (Hrc & Hcsq & Hec & Hex).
    destruct Hex as (ctx & stp & st & Hec_ & Hini).
  ...
  Proof.
    ...
    asserts_rewrite (seller cs = seller_adr).
    {
      lets H__: seller_buyer_arbitrator_addr Hr Hec.
      destruct H__ as (cs_ & Hcs__ & Hb_ & Hs_ & Ha_).
      unfolds in Hcs__. simpl in Hcs__.
      rewrite Eecs in Hcs__. rewrite Hcs in Hcs__. inverts Hcs__. 
      tauto.
    }
    asserts_rewrite ((buyer_adr =? seller_adr)%address = false).
    {
      eapply address_eq_ne; eauto.
      pose proof buyer_seller_neq; eauto.
    }
    simpl.
    asserts_rewrite (buyer cs = buyer_adr).
    {
      lets H__: seller_buyer_arbitrator_addr Hr Hec.
      destruct H__ as (cs_ & Hcs__ & Hb_ & Hs_ & Ha_).
      unfolds in Hcs__. simpl in Hcs__.
